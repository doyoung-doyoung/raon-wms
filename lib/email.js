...
export async function sendLeaveStatusEmail({ to, name, leaveType, startDate, endDate, days, status, rejectedReason, pdfBuffer, pdfFileName }) {
  ...
  if (pdfBuffer && pdfFileName) {
    mailOptions.attachments = [{ filename: pdfFileName, content: pdfBuffer, contentType: 'application/pdf' }]
  }
  await transporter.sendMail(mailOptions)
}

export async function sendExpenseStatusEmail({ to, name, title, category, amount, currency, expenseDate, status, rejectedReason }) {
  const categoryLabel = {
    transport:     '교통비',
    meal:          '식비',
    accommodation: '숙박비',
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f8fafc; padding: 24px; border-radius: 12px;">
      <div style="background: #1e2235; border-radius: 12px; padding: 32px; color: #f1f3f9;">
        <h1 style="margin: 0 0 8px; font-size: 20px; color: #f1f3f9;">RAON Co., Ltd.</h1>
        <p style="margin: 0; color: #8b91ab; font-size: 13px;">경비 청구 결과 안내</p>
      </div>

      <div style="background: white; border-radius: 12px; padding: 32px; margin-top: 12px;">
        <p style="font-size: 16px; color: #1e293b;">안녕하세요, <strong>${name}</strong>님</p>
        <p style="color: #475569;">신청하신 경비 청구가 <strong style="color: ${statusColor};">${statusLabel}</strong>되었습니다.</p>

        <div style="background: #f8fafc; border-radius: 8px; padding: 20px; margin: 20px 0;">
          <table style="width: 100%; border-collapse: collapse;">
            <tr>
              <td style="padding: 8px 0; color: #64748b; font-size: 13px;">항목</td>
              <td style="padding: 8px 0; color: #1e293b; font-weight: 600;">${title}</td>
            </tr>
            <tr>
              <td style="padding: 8px 0; color: #64748b; font-size: 13px;">분류</td>
              <td style="padding: 8px 0; color: #1e293b; font-weight: 600;">${categoryLabel}</td>
            </tr>
            <tr>
              <td style="padding: 8px 0; color: #64748b; font-size: 13px;">금액</td>
              <td style="padding: 8px 0; color: #1e293b; font-weight: 600;">${Number(amount).toLocaleString()} ${currency}</td>
            </tr>
            <tr>
              <td style="padding: 8px 0; color: #64748b; font-size: 13px;">지출일</td>
              <td style="padding: 8px 0; color: #1e293b; font-weight: 600;">${expenseDate}</td>
            </tr>
            <tr>
              <td style="padding: 8px 0; color: #64748b; font-size: 13px;">상태</td>
              <td style="padding: 8px 0; font-weight: 600; color: ${statusColor};">${statusLabel}</td>
  ...
  await transporter.sendMail({
    from: `"RAON WMS" <${process.env.GMAIL_USER}>`,
    to,
    subject: `[RAON] 경비 청구 ${statusLabel} 안내 - ${title} (${Number(amount).toLocaleString()} ${currency})`,
    html,
  })
}
